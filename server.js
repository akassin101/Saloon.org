...
// ─── PHOTO STORE ──────────────────────────────────────────────────────────────
// Profile photos used to live inside the JSON database as base64 data URIs. One
// 2.8 MB photo made GET /api/data a 3.97 MB response for every visitor on every
// page load, and made every unrelated write re-serialise those megabytes. The
// bytes now live as files beside the database on the same volume; the database
// keeps only an extension and a version number.
//
// This block sits ABOVE the migrations that use it: safeImage/DATA_IMAGE_RE are
// const-bound, so a migration referencing them from higher up the file would hit
// the temporal dead zone and crash at boot.

const PHOTOS_DIR = path.join(path.dirname(DATA), 'photos');

// A downscaled profile photo is 25-60 KB. This ceiling is generous for a real
// upload and still small enough that the original problem cannot recur through
// a stale client or a direct API call.
const MAX_PHOTO_BYTES = 400 * 1024;

const IMAGE_EXT  = { png: 'png', jpeg: 'jpg', jpg: 'jpg', gif: 'gif', webp: 'webp' };
const IMAGE_MIME = { png: 'image/png', jpg: 'image/jpeg', gif: 'image/gif', webp: 'image/webp' };

// Ids are server-generated by uid() and always [a-z0-9]. Validated anyway: this
// value reaches the filesystem, and this codebase has shipped a path bug before.
const SAFE_ID_RE = /^[a-z0-9]{1,40}$/;

// Rejects anything that is not a well-formed base64 image. The pattern is linear
// (one character class, one quantifier) so it cannot backtrack pathologically on
// a large input.
...
// Returns { buffer, ext } or null. The extension comes from the validated data
// URI itself, never from a client-supplied filename or header.
function decodeImage(dataUri) {
  const s = safeImage(dataUri);
  if (!s) return null;
  const m = s.match(/^data:image\/(png|jpeg|jpg|gif|webp);base64,(.+)$/);
  if (!m) return null;
  let buffer;
  try { buffer = Buffer.from(m[2], 'base64'); } catch (e) { return null; }
  if (!buffer.length) return null;
  return { buffer, ext: IMAGE_EXT[m[1]] };
}

// Two independent barriers against escaping the photos directory: the id must
// match SAFE_ID_RE, and the resolved path must still sit directly inside
// PHOTOS_DIR. The extension is never taken from a request.
function photoPathFor(userId, ext) {
  if (!SAFE_ID_RE.test(String(userId || ''))) return null;
  if (!Object.values(IMAGE_EXT).includes(ext)) return null;
  const p = path.resolve(PHOTOS_DIR, `${userId}.${ext}`);
  if (path.dirname(p) !== path.resolve(PHOTOS_DIR)) return null;
  return p;
}

function ensurePhotosDir() {
  try {
    fs.mkdirSync(PHOTOS_DIR, { recursive: true, mode: 0o755 });
  } catch (e) {
    console.error(`\n  FATAL: cannot create photos directory at ${PHOTOS_DIR}: ${e.message}\n`);
    process.exit(1);
  }
}
ensurePhotosDir();

// A SIGKILL between writing a temp file and renaming it leaves the temp behind:
// the cleanup in writePhotoFile only runs for a thrown error, not a hard kill.
// Sweep anything left over from a previous life at boot.
(function sweepStalePhotoTemps() {
  try {
    const cutoff = Date.now() - 60 * 60 * 1000;
    fs.readdirSync(PHOTOS_DIR).forEach(name => {
      if (!name.endsWith('.tmp')) return;
      const p = path.join(PHOTOS_DIR, name);
      try {
        if (fs.statSync(p).mtimeMs < cutoff) { fs.unlinkSync(p); console.log(`[photo] swept stale temp ${name}`); }
      } catch (_) {}
    });
  } catch (e) {
    console.warn('[photo] could not sweep temp files:', e.message);
  }
})();

// Writes bytes to the canonical path for a user, atomically. Returns true on
// success. The temp name is unique per call so two concurrent uploads for the
// same user cannot clobber each other's in-flight file.
function writePhotoFile(userId, ext, buffer) {
  const dest = photoPathFor(userId, ext);
  if (!dest) return false;
  const tmp = `${dest}.${process.pid}.${Date.now()}.${Math.random().toString(36).slice(2, 8)}.tmp`;
  try {
    fs.writeFileSync(tmp, buffer, { mode: 0o644 });
    fs.renameSync(tmp, dest);
    return true;
  } catch (e) {
    console.error(`[photo] write failed for ${userId}: ${e.message}`);
    try { fs.unlinkSync(tmp); } catch (_) {}
    return false;
  }
}

function removePhotoFile(user) {
  if (!user || !user.photoExt) return;
  const p = photoPathFor(user.id, user.photoExt);
  if (!p) return;
  fs.unlink(p, err => {
    if (err && err.code !== 'ENOENT')
      console.error(`[photo] failed to delete file for ${user.id}: ${err.message}`);
  });
}
...
// Move inline base64 photos out of the database and onto disk. The bytes are
// preserved, not discarded. Ordering matters: the inline field is only deleted
// AFTER the file write succeeds, so a crash or a full disk mid-migration leaves
// the original data intact and the migration simply retries on the next boot.
if (!db.meta.photosMigratedAt) {
  let moved = 0, failed = 0;
  db.users.forEach(u => {
    if (typeof u.photo !== 'string' || !u.photo) return;
    const decoded = decodeImage(u.photo);
    if (!decoded) {
      // Cannot be served, but must not be destroyed. Park the raw value beside
      // the photos so a corrupt or hand-edited record stays recoverable.
      try {
        const quarantine = path.join(PHOTOS_DIR, `${u.id}.unreadable.txt`);
        fs.writeFileSync(quarantine, String(u.photo), { mode: 0o600 });
        console.warn(`[migration] user ${u.id} had an unreadable inline photo; preserved at ${quarantine}`);
        delete u.photo;
      } catch (e) {
        console.error(`[migration] could not quarantine unreadable photo for ${u.id}: ${e.message}`);
        failed++;   // leave u.photo intact and retry next boot
      }
      return;
    }
    if (writePhotoFile(u.id, decoded.ext, decoded.buffer)) {
      u.photoExt = decoded.ext;
      u.photoVersion = 1;
      u.photoBytes = decoded.buffer.length;
      delete u.photo;
      moved++;
      console.log(`[migration] moved ${decoded.buffer.length} byte photo for ${u.id} to disk.`);
    } else {
      failed++;
    }
  });
  if (failed === 0) {
    db.meta.photosMigratedAt = Date.now();
    if (moved) console.log(`[migration] ${moved} photo(s) moved out of the database.`);
  } else {
    console.error(`[migration] ${failed} photo(s) could not be written; leaving them inline and retrying next boot.`);
  }
}
...
// Photo bytes, served from disk rather than embedded in every API response.
// Public, like the profiles they belong to. The URL is versioned by the client
// (?v=<photoVersion>), so the bytes at a given URL never change and can be
// cached indefinitely.
app.get('/api/users/:id/photo', (req, res) => {
  const user = db.users.find(u => u.id === req.params.id);
  if (!user || !user.photoExt) return res.status(404).json({ error: 'No photo.' });

  const filePath = photoPathFor(user.id, user.photoExt);
  if (!filePath) return res.status(404).json({ error: 'No photo.' });

  fs.stat(filePath, (err, stat) => {
    // A version pointing at a missing file degrades to "no photo" rather than
    // a 500; the client already renders initials in that case.
    if (err || !stat.isFile()) return res.status(404).json({ error: 'No photo.' });

    const etag = `"${user.id}-${user.photoVersion || 0}"`;
...
app.patch('/api/users/:id', requireAuth, (req, res) => {
  ...
  if (b.photo !== undefined) {
    if (b.photo === null || b.photo === '') {
      removePhotoFile(user);
      delete user.photoExt;
      delete user.photoVersion;
      delete user.photoBytes;
      delete user.photo;
    } else {
      if (typeof b.photo !== 'string')
        return res.status(400).json({ error: 'Photo must be an image data URL.' });
      // Bound the base64 string before decoding so a huge body cannot be
      // materialised into a Buffer just to be rejected afterwards.
      // A browser still running the previous frontend uploads at full camera
      // resolution and lands here. Tell that person what to actually do.
      if (b.photo.length > Math.ceil(MAX_PHOTO_BYTES * 4 / 3) + 128)
        return res.status(400).json({ error: 'That photo is too large. Please refresh the page and try again.' });
      const decoded = decodeImage(b.photo);
      if (!decoded)
        return res.status(400).json({ error: 'Photo must be a PNG, JPEG, GIF, or WebP image.' });
      if (decoded.buffer.length > MAX_PHOTO_BYTES)
        return res.status(400).json({ error: 'That photo is too large. Please refresh the page and try again.' });

      const previousExt = user.photoExt;
      // The record is only updated once the bytes are safely on disk, so a
      // failed write can never leave a version pointing at a missing file.
      if (!writePhotoFile(user.id, decoded.ext, decoded.buffer))
        return res.status(500).json({ error: 'Could not save that photo. Please try again.' });

      if (previousExt && previousExt !== decoded.ext)
        removePhotoFile({ id: user.id, photoExt: previousExt });

      user.photoExt = decoded.ext;
      user.photoVersion = (user.photoVersion || 0) + 1;
      user.photoBytes = decoded.buffer.length;
      delete user.photo;
    }
  }
